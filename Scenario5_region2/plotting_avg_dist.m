...
for k = 1:400
for i = 1:2
    filename = sprintf('model_thinned%d_%d.txt',k-1,1200+480*(i-1));
    fileID = fopen(filename);
    j = 1;
    while j <= 301*196
        matData = fgetl(fileID);
        data_CM2(i,j,k) = str2num(matData);
        j=j+1;
    end 
end
end
...
for num = 1:400
        for size_x = 1:301
            for size_y = 1:196
                 ...
                 best_fit(size_x,size_y,2) = best_fit1680(size_y + 196*(size_x-1));
                 best_fit(size_x,size_y,3) = best_fit2160(size_y + 196*(size_x-1));
                 data_thinned(size_x,size_y,1) = data_CM2(1,size_y + 196*(size_x-1),num);
                 data_thinned(size_x,size_y,2) = data_CM2(2,size_y + 196*(size_x-1),num);
                 data_thinned(size_x,size_y,3) = data_CM2(3,size_y + 196*(size_x-1),num);
            end
        end
end
...
best_IC_day2(:,:,1) = best_fit(:,:,1);
best_IC_day2(:,:,2) = data_thinned(:,:,1);
best_IC_day2(:,:,3) = 0;
best_IC_day4(:,:,1) = best_fit(:,:,2);
best_IC_day4(:,:,2) = data_thinned(:,:,2);
best_IC_day4(:,:,3) = 0;
best_IC_day6(:,:,1) = best_fit(:,:,3);
best_IC_day6(:,1,2) = 0;
best_IC_day6(:,:,2) = data_thinned(:,:,3);
best_IC_day6(:,:,3) = 0;


fig1 = figure;
imagesc(best_IC_day2)
axis image;
axis off;
fig2 = figure;
imagesc(best_IC_day4)
axis image;
axis off;
fig3 = figure;
imagesc(best_IC_day6)
axis image;
axis off;